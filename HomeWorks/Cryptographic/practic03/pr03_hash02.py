...
def generate_keypair(curve: EllipticCurve, G: Tuple[int, int], q: int) -> Tuple[int, Tuple[int, int]]:
    ...
    Исключения:
        ValueError: Если генерация ключа не удалась или Q не на кривой.
    """
    max_attempts = 100
    for _ in range(max_attempts):
        ...
        if not curve.is_point_on_curve(Q):
            continue
        return d, Q
    raise ValueError("Не удалось сгенерировать валидную ключевую пару")
...
def verify_signature(message: bytes, signature: Tuple[int, int], curve: EllipticCurve, G: Tuple[int, int], q: int,
    ...
    """
    r, s = signature
    print(f"Проверка подписи: r = {r}, s = {s}")
    if not (0 < r < q and 0 < s < q):
        print(f"Ошибка: r или s вне диапазона (0, {q})")
        return False
    if not curve.is_point_on_curve(Q):
        print(f"Ошибка: открытый ключ Q = {Q} не лежит на эллиптической кривой")
        return False
    h = hash_message(message)
    e = h % q
    if e == 0:
        e = 1
    print(f"Хэш сообщения (h) = {h}, e = h % q = {e}")
    v = mod_inverse(s, q)
    if v is None:
        print("Ошибка: модульное обратное для s не существует")
        return False
    print(f"Модульное обратное v = s^(-1) mod q = {v}")
    z1 = (e * v) % q
    z2 = (r * v) % q
    print(f"z1 = (e * v) % q = {z1}, z2 = (r * v) % q = {z2}")
    P1 = curve.multiply_point(G, z1)
    P2 = curve.multiply_point(Q, z2)
    if P1 is None:
        print(f"Ошибка: z1 * G = None (бесконечная точка)")
        return False
    if P2 is None:
        print(f"Ошибка: z2 * Q = None (бесконечная точка)")
        return False
    R = curve.add_points(P1, P2)
    if R is None:
        print("Ошибка: R = z1 * G + z2 * Q — бесконечная точка")
        return False
    print(f"Точка R = z1 * G + z2 * Q = {R}")
    result = (R[0] % q) == r
    print(f"Сравнение: R[0] % q = {R[0] % q}, r = {r}, результат = {result}")
    return result
