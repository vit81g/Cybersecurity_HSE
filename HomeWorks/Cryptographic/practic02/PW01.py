...
def mod_inverse(e: int, phi: int) -> Optional[int]:
    ...
    Args:
        e (int): Число, для которого ищется обратное.
...
def generate_keypair(bits: int = 512) -> Tuple[Tuple[int, int], Tuple[int, int]]:
    ...
    Args:
        bits (int): Битовая длина модуля n (по умолчанию 512).

    Returns:
        Tuple[Tuple[int, int], Tuple[int, int]]: Открытый ключ (e, n) и закрытый ключ (d, n).

    Raises:
        ValueError: Если не удалось сгенерировать ключи.
    """
...
def encrypt_block(message: int, e: int, n: int) -> int:
    ...
    Returns:
        int: Зашифрованное число.

    Raises:
        ValueError: Если message >= n.
    """
    if message >= n:
...
def decrypt_block(ciphertext: int, d: int, n: int) -> int:
    ...
    Args:
        ...
        d (int): Закрытая экспонента.
        n (int): Модуль.

    Returns:
        int: Расшифрованное число.

    Raises:
        ValueError: Если ciphertext >= n.
    """
    if ciphertext >= n:
