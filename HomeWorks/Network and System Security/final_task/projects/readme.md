...
> **Цель проекта:** изолировать критичные данные МРТ, запустить коммерческий IaaS‑сервис и обеспечить полноценное аварийное восстановление.

---

## Слайд 1 — Почему меняем PPCS

**Спикер‑тезис**
Сегодня: одна плоская L2‑сеть (D‑Link + MikroTik), отсутствие сегментации, шифрования и DR‑площадки.
Завтра:

* 🌐 **Spine/Leaf‑фабрика 40 GbE**
* 🛡️ **Cisco FTD 2140 (HA) + 2 × ISP /eBGP**
* 🔒 **VRF‑10** *(МРТ)* · **VRF‑20** *(SMB)* · **VRF‑30** *(Factory)* · **VRF‑40** *(PPCS‑Core)*
* 🔐 **ГОСТ‑VPN ⇄ HQ**
* 💾 **DR‑Site** — Flex.One × 2, репликация 10 GbE eBGP EVPN
...
## 🔒 Описание зоны защиты персональных данных (152-ФЗ)

### 📍 Локация

* **Зона МРТ** — выделенные 2 стойки внутри ЦОД PPCS, содержащие:

  * 30 × Yadro VEGMAN Rx20 G2;
  * 2 × Flex.One;
  * криптографические шлюзы СКЗИ (Континент 3.9 или ViPNet HW);
  * управляющий коммутатор/Leaf с портами VLAN/VRF-10.

### 📦 Меры физической защиты

| Мера                                     | Реализация                                                                                | Требование                     |
| ---------------------------------------- | ----------------------------------------------------------------------------------------- | ------------------------------ |
| **Отдельная стойка/зона**                | МРТ-оборудование размещено в отдельной стойке с шасси-контролем доступа и опломбированием | 152-ФЗ, Приказ ФСТЭК № 21      |
| **Закрытая металлическая клетка (cage)** | Вокруг стоек МРТ монтируется клетка с замком; ключ — только у доверенных админов          | ТР ИБ, Приказ ФСТЭК № 239      |
| **СКУД**                                 | Доступ в клетку — только по RFID-карте или биометрии, логируется                          | ГОСТ Р 57580.1–2017 (п. 6.3.1) |
| **Видеонаблюдение**                      | Камера с обзором 360°, запись ≥ 90 суток, интеграция в СОТП ЦОД                           | 152-ФЗ, Рекомендации ФСТЭК     |
| **Датчики открытия/вибрации**            | На дверях стоек и клеток, с выводом сигнала в SIEM / NOC                                  | ISO/IEC 27001 A.11             |
| **Раздельное питание и PDU**             | ИБП отдельного канала, визуальная маркировка стоек МРТ (красные бирки)                    | ГОСТ Р 57580.1–2017            |

### 🧾 Документирование

* **Журнал доступа** в клетку: Ф.И.О., дата, цель, отметка администратора МРТ.
* **План размещения** и **паспорт зоны защиты** оформляются в составе комплекта проектной документации (раздел: средства защиты персональных данных).
* При проверке исполнения требований 152-ФЗ наличие физических ограничений — **критически важный пункт**.

<details>
<summary>Сокращения и определения</summary>

| Сокр.    | Расшифровка                                                  | Значение                                   |
| -------- | ------------------------------------------------------------ | ------------------------------------------ |
| **СКЗИ** | Средство криптографической защиты информации                 | сертифицированные устройства шифрования    |
| **СКУД** | Система контроля и управления доступом                       | контроль доступа по RFID / биометрии       |
| **SIEM** | Security Information and Event Management                    | система управления журналами безопасности  |
| **PDU**  | Power Distribution Unit                                      | блок распределения электропитания в стойке |
| **ИБП**  | Источник бесперебойного питания                              | питание оборудования при сбоях сети        |
| **СОТП** | Система обеспечения оперативной технологической безопасности | видеонаблюдение, контроль среды            |
...
Eltex MES5448, Juniper QFX5120-48Y
---
...
Juniper QFX10002-36Q, Huawei CloudEngine CE12804S
---
