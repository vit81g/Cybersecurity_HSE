...
def add_padding(data: bytes, block_size: int = 8) -> bytes:
    ...
    Args:
        ...
        block_size (int): Размер блока в байтах (по умолчанию 8).
    Returns:
        bytes: Данные с дополнением.
    """
...
def remove_padding(data: bytes) -> bytes:
    ...
    Args: data (bytes): Данные с дополнением.
    Returns: bytes: Данные без дополнения.
    Raises: ValueError: Если дополнение некорректно.
    """
    # Проверяем дополнение на корректность
    if not data:
        return data
    padding_length = data[-1]
    # проверяем длину дополнения
    ...
    if not all(b == padding_length for b in data[-padding_length:]):
        raise ValueError("Некорректное дополнение")
    # удаляем дополнение, если оно корректно
...
def process_file(input_file: str, output_file: str, key: bytes, mode: str = "encrypt") -> None:
    ...
    with open(input_file, 'rb') as f_in:
        data = f_in.read()
        # проверяем длину входного файла
        if mode == "encrypt":
        ...
        # расшифровываем
            ...
            for i in range(0, len(data), 8):
                block = data[i:i + 8]
                if len(block) != 8:
                    raise ValueError("Размер входного файла для расшифровки должен быть кратен 8 байтам")
...
def main() -> None:
    ...
    try:
        ...
        # Если это текстовый файл, показываем содержимое расшифрованного файла
        if mode == "decrypt":
